      ...
      case 'SAVE_DATA':
        ...
        if (event.data.data.preserveFields === true && event.data.data.completeData) {
          ...
          case 'ADD_TO_BANK':
            ...
            handleAddToBank(user.id, event.data.data, function(success) {
...
function handlePreserveFieldsDataSave(userId, data, callback) {
  console.log("Flashcard app: Saving with field preservation for user:", userId);
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log("Flashcard app: Successfully saved topic list data:", response.id);
      callback(true);
    },
    error: function(error) {
      console.error("Flashcard app: Error saving topic list data:", error);
      callback(false);
    }
  });
}
...
// Handle adding cards to both the card bank and Box 1
function handleAddToBank(userId, data, callback) {
